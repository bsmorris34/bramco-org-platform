...
def test_account_id_format(terraform_vars):
    ...
    for account_name, account_id in account_ids.items():
        ...
        assert not account_id.startswith('0'), f"{account_name} account ID cannot start with 0"
...
def test_budget_business_rules(terraform_vars):
    ...
    assert budgets["prod"] >= budgets["staging"], "Production budget should be >= staging"
